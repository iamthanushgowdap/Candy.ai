...
export async function getAvailableModels(): Promise<string[]> {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_OLLAMA_URL || process.env.OLLAMA_BASE_URL || "http://127.0.0.1:11434"}/api/tags`, { headers: { "Bypass-Tunnel-Reminder": "true", "ngrok-skip-browser-warning": "true" },
...
export async function getActiveVramUsage(): Promise<{ usageBytes: number; loadedModels: string[] }> {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_OLLAMA_URL || process.env.OLLAMA_BASE_URL || "http://127.0.0.1:11434"}/api/ps`, { headers: { "Bypass-Tunnel-Reminder": "true", "ngrok-skip-browser-warning": "true" },
